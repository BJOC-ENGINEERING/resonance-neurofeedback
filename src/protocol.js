...
export function computeFeatures(spectra, selected) {
  ...
  f.asym = left && right
    ? bandAmplitude(right, 8, 13) / Math.max(0.01, bandAmplitude(left, 8, 13))
    : null;
  return f;
}
...
export class ProtocolEngine {
  ...
  finishCalibration() {
    ...
    const needed = this.protocol.rules.map(r => r.measure);
    if (samples.alpha.length < 10 || needed.some(k => samples[k].length < 10)) return false;
    this.baseline = {};
  ...
  evaluate(features, dt, { valid = true } = {}) {
    ...
    const rows = this.protocol.rules.map(r => {
      ...
      const base = reference?.[r.measure] ?? null;
      const pct = now !== null && base ? (now / base) * 100 : null;
      if (usable && pct !== null && this.calibrated) this.adapt(r, pct);
